...
public class DaoCosplayImpl implements DaoCosplay {
    ...
    public void validateCosplay(Cosplay cosplay) {
        ...
        if (!(searchAct && searchPart)) {
            throw new RuntimeException("You cannot add an activity that is not cosplay or that is not registered, or the participant to add to the event cannot be found");
        }
    ...
    public List<Cosplay> readAll() {
        ...
        try (PreparedStatement preparedStatement = con.prepareStatement(query)) {
            ...
            while (rs.next()) {
                ...
                cosplayBuilderImpl.cosplayId(rs.getInt("cosplayID"))
    ...
    public void modifyCosplay(int id) {
        throw new UnsupportedOperationException("Unimplemented method 'modifyCosplay'");
    }
